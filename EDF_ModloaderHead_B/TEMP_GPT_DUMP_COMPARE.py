...
def toggle_mods_panels(error_msg):
    ...
    window.geometry("675x466")

    # Create frames for file toggling and profiles
    left_frame = tk.Frame(window, padx=10, pady=10, bg=dark_bg, width=400)  # Set width to shrink the left panel
    right_frame = tk.Frame(window, padx=10, pady=10, bg=dark_bg)
    left_frame.pack_propagate(False)
    left_frame.pack(side="left", fill="y")  # Adjust fill to "y" to maintain width control
    right_frame.pack(side="right", fill="both", expand=True)

    # Create a container frame to hold the scrollable file list and the changelog
    scrollable_container = tk.Frame(left_frame, bg=dark_bg)
    scrollable_container.pack(side="top", fill="both", expand=True)

    # Create the canvas and scrollbar specifically for the file list
    file_list_canvas = tk.Canvas(scrollable_container, bg=dark_bg, highlightthickness=0)
    file_scrollbar = tk.Scrollbar(scrollable_container, orient="vertical", command=file_list_canvas.yview)
    scrollable_frame = tk.Frame(file_list_canvas, bg=dark_bg)

    # Configure the scrollable frame inside the canvas
    scrollable_frame.bind(
        "<Configure>",
        lambda e: file_list_canvas.configure(
            scrollregion=file_list_canvas.bbox("all")
        )
    )

    # Create a window inside the canvas to hold the scrollable frame
    file_list_canvas.create_window((0, 0), window=scrollable_frame, anchor="nw")

    # Set the canvas scroll command to the scrollbar
    file_list_canvas.configure(yscrollcommand=file_scrollbar.set)

    # Pack the canvas and scrollbar inside the file list container
    file_list_canvas.pack(side="left", fill="both", expand=True)
    ...
    changelog_text = tk.Text(left_frame, wrap="word", bg=dark_bg, fg=TextColor(), height=10, state=tk.DISABLED)
    changelog_text.pack(fill="x", padx=5, pady=(0, 10))
