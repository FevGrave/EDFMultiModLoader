...
def load_json(file_path):
    ...
    try:
        with open(file_path, "r", encoding="utf-8") as f:
            data = json.load(f)
            # Check for escapes in any string values
            def check_escapes(obj):
                if isinstance(obj, str) and "\\u" in obj:
                    print(f"Warning: Unicode escapes found in '{file_path}': {obj}")
...
def save_json(file_path, data):
    ...
    # Debug: Check for escapes before saving
    def has_escapes(obj):
        if isinstance(obj, str) and "\\u" in obj:
            return True
        elif isinstance(obj, dict):
            return any(has_escapes(v) for v in obj.values())
        elif isinstance(obj, list):
            return any(has_escapes(i) for i in obj)
        return False
    
    if has_escapes(data):
        print(f"Warning: Data contains Unicode escapes before saving to '{file_path}'")
    ...
    for class_name, categories in weapon_tables.items():
        for category, weapons in categories.items():
            for weapon in weapons:
                ...
                data_value = weapon.get("Data", {}).get("value", [])
                if isinstance(data_value, list) and len(data_value) > 0 and isinstance(data_value[0], dict):
                    weapon_name = data_value[0].get("value", "Unknown Weapon")
...
def validate_weapon_entry(entry):
    """Validate the structure of a weapon entry."""
    if not isinstance(entry, dict):
        return False
    if "value" not in entry or not isinstance(entry["value"], list):
        return False
    if len(entry["value"]) <= 8 or not isinstance(entry["value"][8], dict):
        return False
    return True
...
def update_weapon_table(weapon_table, mission_pack_ranks, weapon_name_to_mod):
    ...
    for weapon_entry in weapon_table["variables"][0]["value"]:
        if not validate_weapon_entry(weapon_entry):
            print(f"Skipping invalid weapon entry: {weapon_entry}")
            continue
        weapon_name = weapon_entry["value"][0]["value"]
        if len(weapon_entry["value"]) > 8 and isinstance(weapon_entry["value"][8], dict):
            current_rank = weapon_entry["value"][8].get("value", 3)
        else:
            print(f"Skipping invalid weapon entry: {weapon_entry}")
            continue
