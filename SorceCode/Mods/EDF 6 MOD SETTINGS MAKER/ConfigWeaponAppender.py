...
def process_files(input_dir, import_data, import_text):
    # Define the consistent order of languages
    lang_order = ["CN", "EN", "JA", "KR", "SC"]
    
    # Iterate through mod configuration files
                ...
                for class_name, categories in data["NewToAddWeaponTables"].items():
                    for category, weapons in categories.items():
                        for weapon in weapons:
                            ...
                            # Step 1: Identify languages with non-empty text
                            provided_langs = [lang for lang in lang_order if weapon["Text"].get(lang, [])]
                            
                            # Step 2: Set fallback language
                            if "EN" in provided_langs:
                                # Use "EN" if it has non-empty text
                                fallback_lang = "EN"
                            elif provided_langs:
                                # Otherwise, use the first language with non-empty text
                                fallback_lang = provided_langs[0]
                            else:
                                # If no languages are provided, no fallback
                                fallback_lang = None
                            
                            # Set fallback text based on the chosen language
                            fallback_text = weapon["Text"][fallback_lang] if fallback_lang else []
                            
                            # Step 3: Append text for each language
                            for lang in lang_order:
                                if lang in import_text:
                                    # Use own text if non-empty, else use fallback
                                    text_to_add = weapon["Text"].get(lang, []) or fallback_text
