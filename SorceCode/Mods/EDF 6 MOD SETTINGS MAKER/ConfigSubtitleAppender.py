...
def main(output_directory, current_directory):
    ...
    # Save modified language files and log debug info to files
    for lang_code, data in language_data.items():
        # Log debug info before saving
        if data["variables"][0]["value"]:
            with open('debug_before.log', 'a', encoding='utf-8') as debug_file:
                debug_file.write(f"[DEBUG] Before saving {lang_code}: {data['variables'][0]['value'][1]}\n")
        
        # Save the file
        output_file_path = os.path.join(output_directory, f"EDF6_VOICETABLE.{lang_code}.json")
        save_json_file(output_file_path, data)
        
        # Reload and log debug info after saving
        reloaded_data = load_json_file(output_file_path)
        if reloaded_data["variables"][0]["value"]:
            with open('debug_after.log', 'a', encoding='utf-8') as debug_file:
                debug_file.write(f"[DEBUG] After saving {lang_code}: {reloaded_data['variables'][0]['value'][1]}\n")
        
        # Print confirmation to console (safe, as it avoids non-ASCII data)
