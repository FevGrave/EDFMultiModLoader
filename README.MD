...
Welcome to Earth Defense Force 6 modding! This guide will help you create your first mod configuration file.

1. **Set up your development environment**:
 - Install the latest version of the BA. Mod loader You can do that with the "Update ModLoader (BA)" button.
...
"GITHUB_INFO": [], // GITHUB LINK DOWNLOAD, CAN BE "NEXUS_INFO" Contains metadata about the mod, including author, name, download link, version, and notes. 
 ...
 "DirManifestToFilesUninstaller": {}, // Made with a Tool `ManifestMAKER.exe` that is in the renamed `mymodfolder\Mods` when you package your mod to public downloads you still need to double-check the files
...
- **SGO File**: The game uses a proprietary file format to store various types of data.
...
## Top Row Buttons:
- ### Check for Update:
This button checks for updates to the BA. 'Blue Amulet' Mod loader ensures you're always using the latest version.
- ### Update Mods:
This feature allows you to update the installed mods to their latest versions.
- ### Build Tables:
This button compiles the necessary tables for modding the gameplay, ensuring everything is set up correctly.
- ### Repair Tables:
This option resets the tables to their default settings. It is helpful if you need to revert any changes or fix issues.
- ### Uninstall a Mod:
...
## Main Section:
- ### EDF! Game to Launch:
This section allows you to launch different Earth Defense Force game versions directly from the mod loader.
 - EARTH DEFENSE FORCE 4.1
 - EARTH DEFENSE FORCE 5
 - EARTH DEFENSE FORCE 6
- ### Open Save Folders:
Quickly access the save folders for the different game versions, making it easy to manage your game data. The buttons below indicate the corresponding versions 
 - 4.1, 5, 6.
- ### Modloader Status:
Enabled: Indicates that the winmm.dll mod loader is currently active.
Toggle Modloader Status: A button to enable or disable the winmm.dll mod loader as needed.
- ### Error Section:
Error Block: Displays any errors or logs during the modding process.
...
#### These indicators show how many mods, patches, and plugins are active.
...
Plugins: as .dll advanced external functions 

 EARTH DEFENSE FORCE 6\Mods\Plugins\
# Credits Section:
## Credits to the Developers: Acknowledges the creators behind the mod loader, GUI, and special game patching.
...
FevGrave, X, Reddit: These are direct links to specific user profiles or communities (X refers to the social media platform previously known as Twitter).
...
The `GITHUB_INFO` block stores metadata about the mod, including the mod name, download link, version, and any additional notes.
...
- **LINK:** This is the direct download link for the mod. It should always point to the latest version.

- **VERSION:** The current version of the mod. **Important:** Ensure this version number is constantly updated to reflect the latest release. This is crucial for the auto-download function, as it may soft lock if it attempts to download an outdated version.

- **Notes:** Any additional notes related to the mod. Don't use it as a `CHANGELOG` there is a block for that at the very bottom
...
Constantly update the VERSION field whenever a new mod version is released. Failure to do so may cause the auto-download function to soft-lock if it tries to download a current "outdated" version.
...
- **File:** Specifies which file the replacement should occur in. It must be one of the files listed above.

- **Find** The exact string value to search for in the specified file.

- **Replace:** The new string will replace the found text.
...
Ensure that the Find value matches precisely, including any special characters like newline characters (\n), to avoid unintended changes.

This block is mainly used for patching localization and minor text updates, making it a powerful tool for maintaining consistency across different language versions.

### Use Cases:

This section is ideal for modifying in-game text across different languages to make specific text-based updates without altering the game's core functionality.
...
The `NewToAddTextTableEntries` block adds new text entries to all specified localization files. Due to current limitations, entries are added to all supported language files regardless of the specific content or language.


### Applicable Files:
The entries in this block are added to the following five files:
...
- **2 Letter Lang**
 ...
 - **name:** The identifier for the new text entry. **Important:** Do not use `"GameMode"` or `"MissionPack##"` (# as numbers)for your 2 or more Missionpack Labels as the `name` value, as these are reserved for unique UI setups and could cause UI conflicts.

 - **type:** ALWAYS set to `"string"` for text entries.

 - **value:** The text string that will be added to the specified localization files.
...
- The `name` and `value` fields in `NewToAddTextTableEntries` are also connected to:
 ...
 - **[NewToAddSoldierWeaponCategory Block](#newtoaddsolderweaponcategory-block):** This block's `String` value is tied to the `name` values defined here.
...
This block introduces new text elements necessary for the mod's functionality. Examples might include new menu options, game modes, or item descriptions that weren't present in the original game.
...
The `NewToAddModeList` block defines new game modes or mission packs within the game. This block is particularly important for adding custom offline and online mission packs.

### Structure:

The `NewToAddModeList` block typically contains two central nodes:
...
The `NewToAddSoldierWeaponCategory` block defines new weapon categories for different soldier classes in the game. This block is essential for expanding the range of weapons available to other classes, allowing for greater customization and variety in gameplay.

### Structure:

Each entry in this block typically contains two main components:

1. **Weapon Category ID:** A unique integer identifying the weapon category.
...
- **Weapon Category ID:** An integer value uniquely identifying the weapon category. that corresponds to the `NewToAddWeaponTables` when making a new gun or item to use

- **Weapon Slot Name:** A string value corresponding to the weapon slot's name. This value must match the "name" entry in the `NewToAddTextTableEntries` block.
...
This block introduces new weapon categories for soldier classes, such as the Ranger, Wing Diver, Air Raider, and Fencer. By defining new categories, modders can add custom weapons to the game that fit into these new slots and appear correctly in the soldier classes' weapon selection menus.
...
Each block entry defines the weapon slot and the corresponding category ID or IDs.
...
Ensure the Weapon Slot names are correctly defined and linked to the appropriate category IDs.
...
When adding multiple categories to a single slot, ensure that the slot can support all intended categories to avoid potential conflicts or UI issues.

### Use Cases:

This block organizes and displays the new weapon categories within the soldier classes' weapon selection menus. By correctly configuring this block, modders can control how new weapons appear in the game, ensuring the player has easy access to your provided content.
...
The empty arrays within each category (e.g., "Assault Rifles": []) are placeholders intended to help modders organize weapons. These placeholders allow modders to categorize and structure the guns they're adding, ensuring what they have made will be appended to the end of the game's Weapon Tables.

### Structure:

The block is divided into soldier classes, with each class having its own set of weapon categories:
...
Each soldier class contains a list of weapon categories. These categories are initially filled with placeholders (filler info) to help modders like you organize the weapons. These categories will be appended at the end of the existing lists.
...
The empty arrays within each category (e.g., "Assault Rifles": []) are placeholders intended to help YOU, the modder(s), organize the weapons. These placeholders allow you to categorize and structure the guns you're adding, ensuring what you have made. They will be appended to the end of the game Weapon Tables.
...
Data: This block should contain the weapon data to be added, which Will be appended to the WEAPONTABLE.SGO
...
Organization: The filler information should help you structure the weapon categories. These placeholders will not be used; the data and text will be added as weapons.

Appending: When modders add new weapons, their entries are appended to the end of the existing lists within the game.
...
Below is an example of how a complete entry might look after adding 2 or more new weapons (The second weapon is just a skeleton template):
